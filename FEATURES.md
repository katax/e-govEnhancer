...
- 裁判所公式規則集の民事、刑事、家事・少年、その他にPDF掲載された104文書（重複掲載を統合）をポップアップの通常検索結果に含め、常にLITE表示で開く。
- 専用の起動画面は設けない。
...
- 配布ZIPは `tools/package-extension.ps1` で作成し、カタログ、104件の規則XML、共有カタログスクリプトを含む全必須ファイルがZIP内に存在することを作成直後に検証する。
